...
import {Archive, Clipboard, Layout, LucideIcon, Menu, SlidersHorizontal, User,IndianRupee} from "lucide-react"
...
const sidebarClassNames =`fixed flex flex-col ${isSidebarCollapsed?"w-0 md:w-16":"w-72 md:w-64"} bg-white transition-all duration-300 overflow-hidden h-full shadow-md z-40`;
  return (
    ...
    <div className="flex-grow mt-8">
      ...
      <SidebarLink href="/expenses" icon={IndianRupee} label="Expenses" isCollapsed={isSidebarCollapsed} />
    </div>
    <div className={`${isSidebarCollapsed?"hidden":"block"} mb-10`}>
      <p className="text-center text-xs text-gray-500">&copy; 2024 Shaik Jani</p>
    </div>
    </div>
  )
}
